// Originally derived from: btcsuite/btcd/wire/msgversion.go
...
func (msg *MsgVersion) Decode(r io.Reader) error {
	...
	userAgent, err := bmutil.ReadVarString(r, MaxMessagePayload)
	if err != nil {
		return err
	}
	...
	streamLen, err := bmutil.ReadVarInt(r)
	if err != nil {
		return err
	}
...
func (msg *MsgVersion) Encode(w io.Writer) error {
	...
	err = bmutil.WriteVarString(w, msg.UserAgent)
	if err != nil {
		return err
	}
	...
	for _, stream := range msg.StreamNumbers {
		err = bmutil.WriteVarInt(w, uint64(stream))
		if err != nil {
			return err
		}
	}
...
func (msg *MsgVersion) MaxPayloadLength() int {
	...
	return 4 + 8 + 8 + 26*2 + 8 + bmutil.VarIntSerializeSize(MaxUserAgentLen) +
		MaxUserAgentLen + MaxStreams +
		bmutil.VarIntSerializeSize(MaxStreams)*MaxStreams

...
// ReplaceVersion is used to replace the version field in an identity
// for certain tests.
func ReplaceVersion(pk *identity.PrivateID, v uint64) *identity.PrivateID {
	a, sk, dk := pk.ExportWIF()
	addr, _ := DecodeAddress(a)
	var addrNew Address
	if v == 4 {
		addrNew, _ = NewAddress(v, addr.Stream(), addr.RipeHash())
	} else {
		addrNew, _ = NewDepricatedAddress(v, addr.Stream(), addr.RipeHash())
	}
	pkAddr, _ := identity.ImportWIF(addrNew.String(), sk, dk)
	return identity.NewPrivateID(pkAddr, pk.Behavior(), pk.Pow())
}
...
func TestPubKeys(t *testing.T) {
	...
	// Version 2 address
	{
		v2ID := ReplaceVersion(PrivID1(), 2)
		v2pk := v2ID.PrivateKey()
...

	{
		pubKey2 := TstNewExtendedPubKey(0, time.Now().Add(time.Minute*5).Truncate(time.Second),
			1, 0, SignKey2, EncKey2, 1000, 1000, PrivID2().PrivateKey())
	...
	addr, _ := DecodeAddress("BM-2cTFEueNqmjgR3EqduEZmaZbEW1h9z7M7o")
	...
	undecData, _ := btcec.Encrypt(V5BroadcastDecryptionKey(PrivID1().Address()).PubKey(),
		[]byte{0x00})
...
func TestBroadcasts(t *testing.T) {
	...
	// v4 broadcast
	broadcast2ID := ReplaceVersion(PrivID1(), 3)
	...
	addr, _ := DecodeAddress("BM-2D9ZrtqKDfEWzGaJzkef9d2VnVdNBTg7HL")
	...
	invSigningKey, invEncKey, forwardingData, invalidSig, mismatchSig := TstGenerateBroadcastErrorData(t, validPubkey)
...
func TestMessages(t *testing.T) {
	...
	// SignAndEncryptMsg
	destRipe, _ := hash.NewRipe(PrivID2().Address().RipeHash()[:])
	message, err := TstSignAndEncryptMessage(t, 0, time.Now().Add(time.Minute*5).
		Truncate(time.Second), 1, nil, 4, 1, 1, SignKey1, EncKey1, nil,
		destRipe, 1, []byte("Hey there!"), []byte{}, nil, PrivID1().PrivateKey(), PrivID2().PublicKey())
	if err != nil {
	...
	undecData, _ := btcec.Encrypt(PrivID1().PrivateKey().Decryption.PubKey(),
		[]byte{0x00, 0x00})
	validPubkey, _ := wire.NewPubKey(randId.PubKey().SerializeUncompressed()[1:])

	invDest, invSigningKey, invalidSig, mismatchSig := TstGenerateMessageErrorData(t, validPubkey)

	tests2 := []struct {
		msg    *Message
		privId *identity.PrivateID
	}{
		// Invalid private identity.
		{TstNewTstMessage(t, 0, time.Now().Add(time.Minute*5).Truncate(time.Second),
			1, invPrivID, 4, 1, 1, nil, nil, nil,
			nil, 1, nil, nil, nil), PrivID1()},

		// Decryption failure.
		{TstNewTstMessage(t, 0, time.Now().Add(time.Minute*5).Truncate(time.Second),
			1, []byte{0x00, 0x00}, 4, 1, 1, nil, nil,
			&pow.Data{1000, 1000}, nil, 1,
			nil, nil, nil), PrivID1()},

		// Undecodable decrypted data.
		{TstNewTstMessage(t, 0, time.Now().Add(time.Minute*5).Truncate(time.Second),
			1, undecData, 4, 1, 1, nil, nil, nil, nil, 1,
			nil, nil, nil), PrivID1()},

		// Invalid destination ripe.
		{TstNewTstMessage(t, 0, time.Now().Add(time.Minute*5).Truncate(time.Second),
			1, invDest, 4, 1, 1, nil, nil, nil, nil, 1,
			nil, nil, nil), PrivID1()},

		// Invalid embedded signing key.
		{TstNewTstMessage(t, 0, time.Now().Add(time.Minute*5).Truncate(time.Second),
			1, invSigningKey, 4, 1, 1, nil, nil, nil, nil, 1,
			nil, nil, nil), PrivID1()},

		// Invalid signature.
		{TstNewTstMessage(t, 0, time.Now().Add(time.Minute*5).Truncate(time.Second),
			1, invalidSig, 4, 1, 1, nil, nil, nil, nil, 1,
			nil, nil, nil), PrivID1()},

		// Signature mismatch.
		{TstNewTstMessage(t, 0, time.Now().Add(time.Minute*5).Truncate(time.Second),
			1, mismatchSig, 4, 1, 1, nil, nil, nil, nil, 1,
			nil, nil, nil), PrivID1()},
	}
